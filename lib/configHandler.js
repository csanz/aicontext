...
import { dirTree } from './directoryTree.js';
    ...
    if (fs.existsSync(LEGACY_CONFIG_DIR) && !fs.existsSync(CONFIG_DIR)) {
        ...
        try {
            ...
            if (fs.existsSync(LEGACY_CONFIG_FILE)) {
                fs.copyFileSync(LEGACY_CONFIG_FILE, LEGACY_CONFIG_FILE);
            }
            
            // Migrate exclude file if it exists
            if (fs.existsSync(LEGACY_EXCLUDE_FILE)) {
                fs.copyFileSync(LEGACY_EXCLUDE_FILE, LEGACY_EXCLUDE_FILE);
            }
            ...
            if (fs.existsSync(LEGACY_TEMPLATES_DIR)) {
                ...
                templateFiles.forEach(file => {
                    ...
                    const destPath = path.join(LEGACY_TEMPLATES_DIR, file);
...
function ensureConfigExists() {
    ...
    const configFile = migrationSuccessful ? LEGACY_CONFIG_FILE : LEGACY_CONFIG_FILE;
    const excludeFile = migrationSuccessful ? LEGACY_EXCLUDE_FILE : LEGACY_EXCLUDE_FILE;
