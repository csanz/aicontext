...
/**
 * Validates a glob pattern to ensure it's syntactically correct
...
function validateGlobPattern(pattern) {
    ...
    if (trimmed.includes('***')) {
        return { valid: false, error: 'Invalid pattern: "***" is not valid glob syntax' };
    }

    // Try to compile the pattern with minimatch to catch syntax errors
    try {
        new minimatch.Minimatch(trimmed);
        return { valid: true };
    } catch (err) {
        return { valid: false, error: `Invalid glob pattern: ${err.message}` };
    }
}
 ...
 * @returns {boolean} True if we should use the new config directory, false if we should use legacy
 */
function migrateFromLegacyConfig() {
    ...
    // For fresh installs or when .aicontext exists already, prefer the new directory
    return !fs.existsSync(LEGACY_CONFIG_DIR) || fs.existsSync(CONFIG_DIR);
}
...
function ensureConfigExists() {
    ...
    // For new installations, always prefer the new directory
    // Only fall back to legacy if we specifically had migration issues
    const useNewConfig = migrationSuccessful || !fs.existsSync(LEGACY_CONFIG_DIR);
    
    // Use correct config paths based on preference
    const configDir = useNewConfig ? CONFIG_DIR : LEGACY_CONFIG_DIR;
    const configFile = useNewConfig ? path.join(CONFIG_DIR, 'config.json') : LEGACY_CONFIG_FILE;
    const excludeFile = useNewConfig ? path.join(CONFIG_DIR, 'exclude.json') : LEGACY_EXCLUDE_FILE;
...
function configure() {
    ...
    (async () => {
        try {
            ...
            // Save the configuration to the correct path
            const configPath = fs.existsSync(CONFIG_DIR)
                ? path.join(CONFIG_DIR, 'config.json')
                : LEGACY_CONFIG_FILE;
...
function addExclusion(pattern) {
    // Remove any quotes from the pattern first
    if (pattern) {
...
/**
 * Removes an exclusion pattern by its value from the relative .aicontext/ignore.json file
 *
 * @param {string} pattern - The pattern to remove
 */
function removeExclusionByPattern(pattern) {
    const configFile = ensureRelativeConfigExists();
    let config;
    try {
        config = JSON.parse(fs.readFileSync(configFile.ignoreFile, 'utf8'));
    ...
    const index = config.ignore.indexOf(pattern);
    if (index === -1) {
...
function removeExclusion(index) {
    ...
    if (actualIndex >= 0 && actualIndex < config.ignore.length) {
        ...
        console.error(chalk.red(`Error: Invalid index: ${index}`));
    }
...
function configureIgnore() {
    ...
    readline.question('Enter the number of the pattern to remove (or 0 to cancel): ', (answer) => {
        ...
        if (index === 0) {
            ...
            console.error(chalk.red(`Error: Invalid input '${answer}'. Enter a number between 0 and ${exclusions.patterns.length}.`));
        } else {
...
/**
 * Gets the current include patterns from the relative .aicontext/ignore.json file
 *
 * @returns {Object} The current include patterns
 */
function getInclusions() {
    const configFile = ensureRelativeConfigExists();
    try {
        const config = JSON.parse(fs.readFileSync(configFile.ignoreFile, 'utf8'));
        const patterns = Array.isArray(config.include) ? config.include : [];
        const validPatterns = patterns.filter(pattern => typeof pattern === 'string' && pattern.trim().length > 0);
        return { patterns: validPatterns };
    } catch (error) {
        return { patterns: [] };
...
/**
 * Adds a new include pattern to the relative .aicontext/ignore.json file
 *
 * @param {string} pattern - The pattern to include
 */
function addInclusion(pattern) {
    // Remove any quotes from the pattern first
    if (pattern) {
        pattern = pattern.replace(/^["']|["']$/g, '');
    }
    ...
    if (!config.include.includes(pattern)) {
        ...
        fs.writeFileSync(configFile.ignoreFile, JSON.stringify(config, null, 2));
        console.log(chalk.green(`Added include pattern: ${pattern}`));
    } else {
...
/**
 * Removes an include pattern by its value
 *
 * @param {string} pattern - The pattern to remove
 */
function removeInclusionByPattern(pattern) {
    const configFile = ensureRelativeConfigExists();
    let config;
    try {
        config = JSON.parse(fs.readFileSync(configFile.ignoreFile, 'utf8'));
    ...
    if (!Array.isArray(config.include)) {
        console.log(chalk.yellow('No include patterns to remove'));
        return;
    }

    const index = config.include.indexOf(pattern);
    if (index === -1) {
        console.log(chalk.yellow(`Pattern not found: ${pattern}`));
        return;
    }
    ...
    fs.writeFileSync(configFile.ignoreFile, JSON.stringify(config, null, 2));
    console.log(chalk.green(`Removed include pattern: ${pattern}`));
}

/**
 * Displays the current include patterns to the console
 */
function showInclusions() {
    ...
    if (!config.include || config.include.length === 0) {
        console.log('No include patterns defined (all matching files will be included).');
        console.log('Add patterns using: cx include "pattern"');
    } else {
        console.log(chalk.cyan('Only files matching these patterns will be included:'));
...
function clearInclusions() {
    ...
    config.include = [];
    fs.writeFileSync(configFile.ignoreFile, JSON.stringify(config, null, 2));
    console.log(chalk.green('Cleared all include patterns'));
    console.log(`  (Updated: ${path.relative(process.cwd(), configFile.ignoreFile)})`);
    return { patterns: [] };
}
