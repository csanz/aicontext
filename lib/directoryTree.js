    ...
    try {
        ...
        const items = files
            .filter(file => {
                ...
                // Skip node_modules and other directories in IGNORED_DIRS
                if (stat.isDirectory() && IGNORED_DIRS.includes(file)) return false;
                
                // Skip binary files
                if (!stat.isDirectory() && BINARY_EXTENSIONS.includes(ext)) return false;
                
                // Skip ignored files
                if (!stat.isDirectory() && IGNORED_FILES.includes(file)) return false;
