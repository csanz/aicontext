...
import { getOutputFilePath, getFormatExtension } from './pathUtils.js';
...
import { filterChangedFiles, saveRunTime, formatRelativeTime } from './changeTracker.js';
import { getInclusions } from './configHandler.js';
 ...
 * - Words and numbers are usually 1 token
 * - Punctuation often becomes separate tokens
 * - Code has more punctuation than prose, so ~3.5 chars/token for code
 *
...
function countTokens(content) {
    ...
    // Estimate: words + punctuation (often separate tokens) + some newlines
    // This gives a more accurate estimate for code than simple char/4
    const estimate = words.length + Math.ceil(punctuation.length * 0.7) + Math.ceil(newlines * 0.3);

    // Fallback to char/4 if our estimate seems too low
    const charEstimate = Math.round(content.length / 4);

    return Math.max(estimate, charEstimate);
}
 ...
 * @param {string} options.since - Only include files changed since this time (e.g., "2h", "1d")
 * @param {string} options.gitDiff - Only include files changed vs this git ref (e.g., "main")
 * @param {boolean} options.changed - Only include files changed since last cx run
...
export async function generateContext(options) {
    ...
    try {
        ...
        // Apply incremental filtering (--since, --git-diff, --changed)
        let filterInfo = null;
        if (since || gitDiff || changed) {
            if (spinner) spinner.text = 'Filtering changed files...';
            const filterResult = filterChangedFiles(allFiles, { since, gitDiff, changed });
            allFiles = filterResult.filteredFiles;
            filterInfo = filterResult.filterInfo;

            if (filterInfo) {
                verboseLog(`Filter: ${filterInfo.description}`);
                verboseLog(`Filtered to ${filterInfo.filteredCount} of ${filterInfo.totalFiles} files`);
            }
        }

        // Apply include pattern filtering (whitelist)
        let includeInfo = null;
        const { patterns: userIncludePatterns } = getInclusions();
        if (userIncludePatterns && userIncludePatterns.length > 0) {
            if (spinner) spinner.text = 'Applying include patterns...';
            const totalBeforeInclude = allFiles.length;
            ...
            allFiles = allFiles.filter(file => {
                ...
                return userIncludePatterns.some(pattern => {
            ...
            includeInfo = {
                patterns: userIncludePatterns,
                totalFiles: totalBeforeInclude,
                filteredCount: allFiles.length
            };

            verboseLog(`Include patterns: ${userIncludePatterns.join(', ')}`);
            verboseLog(`Filtered to ${allFiles.length} of ${totalBeforeInclude} files`);
        }

        // Warn if no files were found
        if (allFiles.length === 0) {
            if (spinner) spinner.stop();
            console.log(chalk.yellow('\nWarning: No files found matching your criteria.'));
            if (userIncludePatterns && userIncludePatterns.length > 0) {
                console.log(chalk.gray(`  Include patterns: ${userIncludePatterns.join(', ')}`));
            }
            console.log(chalk.gray('  Check your include/ignore patterns or try a different path.\n'));
            return null;
        }
        ...
        for (let i = 0; i < filesToProcess.length; i++) {
            ...
            try {
                ...
                totalSizeBytes += fileContent.length; // Use content length (already have it)
        ...
        const latestExtension = getFormatExtension(format);
        const latestPath = path.join(dir, `latest-context${latestExtension}`);
        ...
        console.log('');
        console.log(chalk.gray('✔ Context file successfully generated'));

        // Print filter info if applicable
        if (filterInfo && filterInfo.filteredCount !== filterInfo.totalFiles) {
            console.log('');
            console.log(chalk.cyan(`🔍 Incremental Mode: ${filterInfo.description}`));
            console.log(chalk.gray(`   Changed files: ${filterInfo.filteredCount} of ${filterInfo.totalFiles} total`));
        }

        // Print include pattern info if applicable
        if (includeInfo) {
            console.log('');
            console.log(chalk.cyan(`📋 Include Patterns: ${includeInfo.patterns.join(', ')}`));
            console.log(chalk.gray(`   Matched files: ${includeInfo.filteredCount} of ${includeInfo.totalFiles} total`));
        }
        ...
        if (config.autoClipboard && !skipClipboard) {
            try {
                ...
                console.log(chalk.gray('  Clipboard: ✔ copied'));
            } catch (error) {
                console.log(chalk.gray('  Clipboard: failed (file saved successfully)'));
            }
        }

        console.log(''); // Final newline

        // Save run time for --changed option
        saveRunTime();
