...
// Define all known options (including aliases)
const knownOptions = [
    // Commands
    'configure', 'show', 'clear', 'clear-all', 'ignore', 'load-cursor-rules',
    // Ignore subcommands
    'add', 'pattern',
...
export function parseArguments(config) {
    ...
    const parser = yargs(hideBin(process.argv))
        ...
        // Commands
        .command('configure', 'Set up configuration', {}, (argv) => {
            require('./configHandler').configure();
            process.exit(0);
        })
        .command('show', 'Show current configuration', {}, (argv) => {
            require('./configHandler').showConfig();
            process.exit(0);
        })
        .command('clear', 'Remove generated context files', {}, (argv) => {
            require('./cleanupUtils').clearContextFiles({ includeSnapshots: argv.s });
            process.exit(0);
        })
        .command('clear-all', 'Remove all context files and directories', {}, (argv) => {
            require('./cleanupUtils').clearContextFiles({ all: true });
            process.exit(0);
        })
        ...
        .command('load-cursor-rules', 'Load and import cursor rules', {}, (argv) => {
            require('./cursorRulesHandler').loadCursorRules();
            process.exit(0);
        })
