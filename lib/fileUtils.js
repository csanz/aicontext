...
function shouldProcessFile(filePath, options = {}) {
    ...
    // CRITICAL: Explicitly check for node_modules directory
    // This directory contains dependencies and should always be excluded
...
async function findFiles(options) {
    ...
    const searchPromise = new Promise(resolve => {
        ...
        const traverse = (currentDir, currentDepth = 0) => {
            ...
            // CRITICAL: Explicitly check for node_modules directory at any level
            // This is important as node_modules can be very large and contain many files
            if (currentDir.includes('/node_modules/') || 
                currentDir.includes('\\node_modules\\') || 
                path.basename(currentDir) === 'node_modules') {
                verboseLog(`Skipping node_modules directory: ${currentDir}`);
                return;
            }
