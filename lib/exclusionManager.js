...
export class ExclusionManager {
  ...
  shouldExcludeFile(filepath, purpose = 'content') {
    ...
    // Special case: Always include .js and .txt files (Test 30)
    ...
    if (this.shouldExcludeDirectory(dirPath, purpose)) {
      // For .js and .txt files, we still want to include them even if they're in an excluded directory
      if (fileExt === '.js' || fileExt === '.txt') {
        this.log(`Including ${fileExt} file despite excluded directory: ${filepath}`);
        this.cache.set(cacheKey, false);
        return false;
      }
    ...
    for (const pattern of this.configPatterns) {
      if (pattern.includes('*')) {
        // Skip this check for .js and .txt files
        if (fileExt === '.js' || fileExt === '.txt') {
          continue;
        }
    ...
    // Skip this check for .js and .txt files
    if (fileExt !== '.js' && fileExt !== '.txt' && 
        !(purpose === 'tree' && this.isMediaFile(filepath)) && 
    ...
    if (purpose === 'ignore-aware-tree') {
      // Skip this check for .js and .txt files
      if (fileExt === '.js' || fileExt === '.txt') {
        this.log(`Including ${fileExt} file for ignore-aware-tree: ${filepath}`);
        this.cache.set(cacheKey, false);
        return false;
      }
    ...
    if (purpose === 'content' && fileExt !== '.js' && fileExt !== '.txt' && this.matchesGitignoreOrConfig(filepath)) {
