...
export class ExclusionManager {
   ...
   * @param {string} purpose - Purpose of the operation ('content' or 'tree')
  ...
  matchesUserDefinedPatterns(filePath, purpose = 'content') {
    ...
    // For 'ignore-aware-tree' purpose, be especially strict with pattern matching
    if (purpose === 'ignore-aware-tree') {
      // Apply more strict matching for extension patterns
    ...
    // Check exact file extension match for common patterns like *.md
    if (fileExt && this.configPatterns.has(`*.${fileExt.substring(1)}`)) {
      this.log(`Excluding ${filePath} due to extension pattern: *.${fileExt.substring(1)}`);
      return true;
    }
    ...
    for (const pattern of this.configPatterns) {
      ...
      // For extension patterns like *.md, do a simple string check first
      if (pattern.startsWith('*.') && fileExt && fileExt.substring(1) === pattern.substring(2)) {
      ...
      if (matchesRelative) {
        this.log(`Excluding ${relativePath} due to user pattern: ${pattern}`);
        return true;
      }
      
      if (matchesFileName) {
        this.log(`Excluding ${fileName} due to user pattern: ${pattern}`);
        return true;
      }
    }
  ...
  shouldExcludeDirectory(dirPath, purpose = 'content') {
    ...
    if (this.matchesUserDefinedPatterns(dirPath, purpose)) {
      this.log(`Excluding directory by user-defined pattern: ${dirPath}`);
      this.cache.set(cacheKey, true);
      return true;
    }
