...
/**
 * Validate and sanitize a git ref to prevent command injection
 * @param {string} gitRef - The git reference (branch, tag, commit SHA)
 * @returns {string|null} - Sanitized ref or null if invalid
 */
function sanitizeGitRef(gitRef) {
    if (!gitRef || typeof gitRef !== 'string') return null;

    // Git refs can contain: alphanumeric, -, _, /, ., ~, ^
    // They cannot contain: shell metacharacters like ; | & $ ` etc.
    const validRefPattern = /^[a-zA-Z0-9_.\-\/~^@]+$/;

    const trimmed = gitRef.trim();
    if (!validRefPattern.test(trimmed)) {
        console.error(`Error: Invalid git reference "${gitRef}". Contains invalid characters.`);
        return null;
    }

    // Additional safety: reject refs that look like shell commands
    if (trimmed.includes('..') && trimmed.includes('/')) {
        // Allow HEAD~5, branch..branch, but be cautious
    }
...
export function getGitChangedFiles(gitRef) {
    ...
    try {
        ...
        try {
            ...
            const mergeBase = execSync(`git merge-base HEAD ${safeRef}`, {
                encoding: 'utf8',
